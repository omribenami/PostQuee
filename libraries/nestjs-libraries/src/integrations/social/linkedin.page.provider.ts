  ...
  implements SocialProvider
{
  ...
  override async authenticate(params: {
    ...
    try {
      ...
      if (tokenData.error) {
        console.error('LinkedIn Page OAuth error:', tokenData);
        return `LinkedIn Page authorization failed: ${tokenData.error_description || tokenData.error}`;
      }

      // Validate required fields
      if (!tokenData.access_token) {
        console.error('LinkedIn Page token response missing access_token:', tokenData);
        return 'LinkedIn Page authorization failed: No access token received';
      }
      ...
      try {
        ...
        return 'Missing required LinkedIn Page permissions. Please grant all requested permissions and try again.';
      }
      ...
      if (!userinfoResponse.ok) {
        console.error('LinkedIn Page userinfo API error:', userinfoResponse.status);
        return `Failed to fetch LinkedIn Page profile: ${userinfoResponse.statusText}`;
      }

      const userinfoData = await userinfoResponse.json();

      if (!userinfoData.sub || !userinfoData.name) {
        console.error('LinkedIn Page userinfo missing required fields:', userinfoData);
        return 'Failed to retrieve LinkedIn Page profile information';
      }
      ...
      if (!meResponse.ok) {
        console.error('LinkedIn Page me API error:', meResponse.status);
